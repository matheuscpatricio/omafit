 ...
 * Python `run_recipe.py` só com AR_MESH_NODE_RUN_RECIPE_PYTHON=1 e Python disponível.
 */
...
export async function postprocessRodinGlassesGlbBuffer(glbBuf, opts = {}) {
  ...
  if (recipe === "glasses_canonical" && !usePythonRunRecipe()) {
    try {
  ...
  if (!pyProbe.ok) {
    if (recipe === "glasses_canonical") {
      console.warn("[ar-eyewear] Python indisponível — fallback Node glasses_canonical");
      return postprocessGlassesCanonicalNodeBuffer(glbBuf, params);
    }
  ...
  try {
    ...
    if (recipe === "glasses_canonical") {
      console.warn(
        "[ar-eyewear] run_recipe.py falhou — fallback Node glasses_canonical:",
        e?.message || e,
      );
      return postprocessGlassesCanonicalNodeBuffer(glbBuf, params);
    }
