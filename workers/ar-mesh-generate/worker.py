#!/usr/bin/env python3
"""
Consome fila ar_eyewear_assets (status=queued) no Supabase.
Geração 3D: Rodin (fal.ai) por wearableClass, pós-processo por recipe, manifest AR v1.
Fallback: TripoSR local, FAL Tripo, stub.
...
Env:
  ...
  AR_3D_PROVIDER — "rodin" (default) | "tripo" | "triposr" | "fal"
  GENERATION_PROVIDER — alias por job (coluna generation_provider)
...
from manifest.build import build_ar_manifest, manifest_to_json_bytes  # noqa: E402
from presets import get_class_preset, resolve_wearable_class  # noqa: E402
from providers.rodin_fal import run_rodin_fal  # noqa: E402
from providers.tripo_fal import run_tripo_fal  # noqa: E402
...
def _provider(job_provider: str | None = None) -> str:
    jp = str(job_provider or "").strip().lower()
    if jp:
        return jp
    return str(os.environ.get("AR_3D_PROVIDER", "rodin")).strip().lower()


def _patch_stage(row_id: str, stage: str, extra: dict | None = None):
    ...
    try:
        ...
        print(f"[worker] generation_stage patch ignorado ({stage}): {e}", file=sys.stderr)
...
def _collect_image_urls(front_url: str, three_url: str, profile_url: str) -> list[str]:
    urls: list[str] = []
    for u in (front_url, three_url, profile_url):
        s = str(u or "").strip()
...
def _image_urls_from_row(row: dict) -> list[str]:
    ...
    if isinstance(raw, list):
        urls = [str(u).strip() for u in raw if str(u or "").strip()]
    elif isinstance(raw, str) and raw.strip().startswith("["):
        try:
            parsed = json.loads(raw)
            if isinstance(parsed, list):
                urls = [str(u).strip() for u in parsed if str(u or "").strip()]
        except Exception:
            pass
    if urls:
        seen: set[str] = set()
        ...
        for u in urls:
            ...
            if len(out) >= 5:
                break
        return out
    front = str(row.get("image_front_url") or "").strip()
    three = str(row.get("image_three_quarter_url") or "").strip() or front
    profile = str(row.get("image_profile_url") or "").strip() or front
    return _collect_image_urls(front, three, profile)
...
def process_job(row: dict):
    ...
    if not image_urls:
        raise ValueError("job sem imagens de referência")
    front_url = image_urls[0]
    three_url = image_urls[1] if len(image_urls) > 1 else front_url
    profile_url = image_urls[2] if len(image_urls) > 2 else front_url

    wearable_class = resolve_wearable_class(
        wearable_class=row.get("wearable_class"),
        accessory_type=row.get("accessory_type"),
        lens_profile=row.get("lens_profile"),
    )
    preset = get_class_preset(wearable_class)
    ...
    try:
        _patch_stage(
            ...
            "rodin_running",
            {
                ...
                "generation_provider": preset.get("generation_provider"),
            },
        )
        ...
        glb_raw = tmp / "raw_model.glb"
        glb_final = tmp / "model.glb"

        if os.environ.get("WORKER_STUB") == "1":
            stub_glb(glb_raw)
        else:
            provider = _provider(row.get("generation_provider") or preset.get("generation_provider"))
            if provider in ("rodin", "hyper3d"):
                fal_request_id = run_rodin_fal(
                    preset.get("rodin") or {},
                    image_urls,
                    glb_raw,
                )
            elif provider in ("fal", "tripo"):
                fal_request_id = run_tripo_fal(front_url, glb_raw)
            else:
                out_dir = tmp / "tri_out"
                ...
                shutil.copy2(mesh, glb_raw)

        _patch_stage(row_id, "postprocess")
        pp_dir = str(_ROOT / "postprocess")
        pp_timeout = _float_env("POSTPROCESS_TIMEOUT_SECONDS", 900.0)
        pp = _run_cmd_with_timeout(
            [
                sys.executable,
                str(_ROOT / "postprocess" / "run_recipe.py"),
                recipe,
                str(glb_raw),
                str(glb_final),
                json.dumps(recipe_params),
            ],
            cwd=pp_dir,
            env={**os.environ, "PYTHONUNBUFFERED": "1"},
            timeout_sec=pp_timeout,
            timeout_label="run_recipe.py",
        )
        if pp.returncode != 0:
            raise RuntimeError(
                f"run_recipe exit {pp.returncode}\n{_subprocess_fail_detail(pp)}"
            )

        _patch_stage(row_id, "validating")
        data = glb_final.read_bytes()
        storage_base = f"{shop.replace('@', '_')}/{row_id}"
        glb_path = f"{storage_base}/model.glb"
        public_url = upload_storage(glb_path, data, "model/gltf-binary")

        manifest = build_ar_manifest(
            wearable_class=wearable_class,
            preset=preset,
            glb_url=public_url,
            shop_domain=shop,
            asset_id=str(row_id),
            lens_profile=lens_profile_manifest,
        )
        manifest_bytes = manifest_to_json_bytes(manifest)
        manifest_path = f"{storage_base}/ar-manifest.json"
        manifest_url = upload_storage(
        ...
        patch_row(
            row_id,
            {
                ...
                "ar_manifest_draft_url": manifest_url,
                "wearable_class": wearable_class,
                "generation_provider": preset.get("generation_provider") or "rodin",
                "generation_request_id": fal_request_id or None,
                "generation_stage": "completed",
                "error_message": None,
            },
        )
        print(f"[worker] OK {row_id} class={wearable_class} -> {public_url}")
    except Exception as e:
        msg = str(e)[:12000]
...
def main():
    ...
    base = supabase_base()
    print(
        "[worker] ar-mesh-generate started; stub=",
